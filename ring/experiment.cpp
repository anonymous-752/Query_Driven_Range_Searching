...
int main(int argc,char * argv[]){
    ...
    for (int i = 0; i < d; ++i) {
        stddev->x[i] = sqrt(stddev->x[i] / 90000);
    }
    ...
    string resfilename="newtree_";
    string outfile="newquerytimes_";
    int sti=0;
    ...
    for(int i=samplesize;i<60000;i++){
        root->add(dataset[i],i);
    }
    ...
    for(int i=0;i<90000;i++){
        ...
        tottimenano+=double(chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count())/double(1.0);

    }
    tottimenano/=90000.0;

    fout<<"Mean query time per answer: "<<tottimenano<<"[ns]"<<endl;
    */
}
